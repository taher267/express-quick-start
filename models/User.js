...
import { compare } from "jsonwebtoken";
...
userSchema.methods.getJWTToken = function () {
    return sign({ _id: this._id }, process.env.JWT_SECRET, { expiresIn: process.env.JWT_EXPIRY })
}
...
userSchema.methods.getResetPasswordToken = function () {
    ...
    const expiry = process.env.RESET_PASSWORD_EXPIRY ? parseInt(process.env.RESET_PASSWORD_EXPIRY) : 5;

    this.resetPasswordExpire = Date.now() + expiry * 60 * 1000;
    return resetToken;
}
export default model('People', userSchema);